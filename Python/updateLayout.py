...
def updateLayout(layoutName):
    ...
    myLyt = QgsProject.instance().layoutManager().layoutByName(layoutName)
    if myLyt is None:
        return None
    ...
    mapLayers = [[(mItem.displayName(), m.dataProvider().dataSourceUri()) 
                  for m in mItem.layers() if isinstance(m,QgsRasterLayer)] 
                  for mItem in mapItems]
    mapLayers = [m[0] for m in mapLayers if len(m)>0]
    ...
    allMetaData = [(m[0], [mm.split("_") for mm in m[1]]) for m in metaData if len(m[1])>0]
    ...
    sensors = list(set([s[1] for s in assets if len(s) > 0]))
    ...
    copyright_item = myLyt.itemById("Copyright")
    ...
    for candidate in allDateData:
        candidate_item = myLyt.itemById(candidate[0] + "Date")
        if candidate_item is not None:
            candidate_item.setText(DateText % candidate[1])
        else:
            """ Create a new date label and insert in the right place """
            candidate_map = myLyt.itemById(candidate[0])
            newLabel = QgsLayoutItemLabel(myLyt)
            newLabel.setFont(QFont("Arial", 8))
            newLabel.setMode(0)
            newLabel.setHAlign(2)
            newLabel.setText(candidate[1] + "  ")
            newLabel.adjustSizeToText()
            newLabel.setMode(1)
            newLabel.setText(DateText % candidate[1])
            map_xy = candidate_map.pagePos()
            map_hw = candidate_map.sizeWithUnits()
            lbl_hw = newLabel.sizeWithUnits()
            item_pad = 0.5
            newLabel_pos = QPointF(map_xy.x() + map_hw.width() - lbl_hw.width() - item_pad, 
                                   map_xy.y() + map_hw.height() - lbl_hw.height() - item_pad)
            newLabel.setPos(newLabel_pos)
            newLabel.setId(candidate[0] + "Date")
            myLyt.addLayoutItem(newLabel)
